...
# === Transient flash 队列（据点指挥官事件触发）===
var _flash_entries: Array = []  # [{world_pos, color, remaining, duration}]
...
func _process(delta: float) -> void:
	...
	if _update_timer >= _update_interval:
		...
		queue_redraw()
	# Flash 衰减（每帧递减）
	if not _flash_entries.is_empty():
		var to_remove: Array = []
		for entry in _flash_entries:
			entry.remaining -= delta
			if entry.remaining <= 0.0:
				to_remove.append(entry)
		for entry in to_remove:
			_flash_entries.erase(entry)
...
# 据点指挥官事件触发的小地图脉冲（向外扩散淡出）
func _draw_flashes() -> void:
	for entry in _flash_entries:
		var ratio: float = entry.remaining / entry.duration  # 1→0
		var mp := world_to_minimap(entry.world_pos)
		if not _in_bounds(mp):
			continue
		var c: Color = entry.color
		var radius := 8.0 * (1.0 - ratio) + 2.5
		# 外圈淡光
		draw_circle(mp, radius + 3.0, Color(c.r, c.g, c.b, ratio * 0.3))
		# 实心点
		draw_circle(mp, radius * 0.5, Color(c.r, c.g, c.b, ratio))


## 外部 API：在世界坐标 world_pos 触发一个 duration_sec 秒的脉冲
func flash_at(world_pos: Vector2, color: Color, duration_sec: float = 1.5) -> void:
	_flash_entries.append({
		"world_pos": world_pos,
		"color": color,
